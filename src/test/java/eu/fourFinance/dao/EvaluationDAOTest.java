...
public class EvaluationDAOTest extends BaseTest {
    ...
    public void testCreate() {
        ...
        Evaluation s = evaluationDAO.createEvaluation(subject, date, debt, term, rate, requestIP, calculatedCoef);
    ...
    public void testGetSubjectEvaluation() {
        evaluationDAO.createEvaluation(subject, new Date(), 1000d, 5, 1d, "a", 0d);
        evaluationDAO.createEvaluation(subject, new Date(), 2000d, 5, 1d, "a", 0d);
    ...
    public void testCountGivenLoan() {
        ...
        debtsDAO.createDebt(evaluationDAO.createEvaluation(subject, date, 2000d, 5, 1d, ip, 0d));

        Date from = DateUtils.getDateDayStart(date);
        Date till = DateUtils.getDateDayEnd(date);
        ;
        assertEquals(2, evaluationDAO.countGivenLoan(ip, from, till).longValue());
    }
